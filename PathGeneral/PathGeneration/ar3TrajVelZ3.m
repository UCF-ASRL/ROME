%% Jacob's Take on ar2TrajVelZ3
function [path,pathDeg] = ar3TrajVelZ3(time,initialtheta,posTraj, velTraj)
...
scale=1;
posTraj=posTraj*scale;
velTraj=velTraj*scale;

%{
scaleV=1;
%refTraj(:,2)=refTraj(:,2)*scaleV;
%convert velocities 3000x6 reftraj version
for i=4:6
    refTraj(:,i)=refTraj(:,i)*scaleV;
end
%}
%% 
...
%       Forward Integrate to find Angles
        for j=1:6
            theta(i+1,j)=theta(i,j)+(thetad(i,j)*ts(i)); 
        end
