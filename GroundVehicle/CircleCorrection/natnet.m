	...
	methods ( Access = public  )
		...
		function modelDescription = getModelDescription( obj )
			if ( obj.IsConnected == 1 )
				...
				modelDescription.TrainedMarkersetCount = 0;
				...
				for i = 1 : modelDescription.TrackingModelCount
					...
					elseif (descriptor.type == 6)
						modelDescription.TrainedMarkersetCount = modelDescription.TrainedMarkersetCount + 1
						modelDescription.TrainedMarkerset(modelDescription.TrainedMarkersetCount).Name = char( descriptor.Name );
						modelDescription.TrainedMarkerset(modelDescription.TrainedMarkersetCount).ID = descriptor.AssetID;
						modelDescription.TrainedMarkerset(modelDescription.TrainedMarkersetCount).BoneCount = descriptor.nRigidBodies;
						for i = 1: modelDescription.TrainedMarkerset(modelDescription.TrainedMarkersetCount).BoneCount
							modelDescription.TrainedMarkerset(modelDescription.TrainedMarkersetCount).Bone(i).Name = char(descriptor.RigidBodies(i).Name);
							modelDescription.TrainedMarkerset(modelDescription.TrainedMarkersetCount).Bone(i).ParentID = descriptor.RigidBodies(i).parentID;
							modelDescription.TrainedMarkerset(modelDescription.TrainedMarkersetCount).Bone(i).OffsetX = descriptor.RigidBodies( i ).offsetx;
							modelDescription.TrainedMarkerset(modelDescription.TrainedMarkersetCount).Bone(i).OffsetY = descriptor.RigidBodies( i ).offsety;
							modelDescription.TrainedMarkerset(modelDescription.TrainedMarkersetCount).Bone(i).OffsetZ = descriptor.RigidBodies( i ).offsetz;
							modelDescription.TrainedMarkerset(modelDescription.TrainedMarkersetCount).Bone(i).Type = descriptor.RigidBodies( i ).type;
						end
				...
				report( obj , [ 'number of trained markersets: ', num2str(modelDescription.TrainedMarkersetCount)])
			else
		...
		function frameOfMetaData = getFrameMetaData( obj )	
			...
			if ( obj.IsConnected == 1 )
				...
				t.TrainedMarkersetCount = data.nAssets;
				frameOfMetaData = t;
			else
				obj.report( 'connection not established' )
			end
