...
vx_cap_log = zeros(1,numFrames);
vy_cap_log = zeros(1,numFrames);
...
for idx = 2:numFrames
    ...
    %% Calculate velocity
    vx_cap = (x_cap - x_cap_prev)/dt;
    vy_cap = -(y_cap - y_cap_prev)/dt;

    vx_cap_log(idx) = vx_cap; vy_cap_log(idx) = vy_cap;
    ...
    vx_corr = Kpx*ex + Kix*int_ex + Kdx*(ex - prev_ex)/dt;
    vy_corr = Kpy*ey + Kiy*int_ey + Kdy*(ey - prev_ey)/dt;
    ...
    %% --- Map to wheel speeds --- %%
    w_cmd_rads = InverseKinematics([vx; vy; yaw_cap]);
...
% Plot commanded vs tracked body velocities
figure;
hold on
plot(time_s, vx_ref_log(:), 'b', 'LineWidth', 4)
plot(time_s, vx_cap_log(:), 'r', 'LineWidth', 2)
hold off
title('Commanded vs Tracked X-Velocity')
xlabel('Time (sec)')
...
figure;
hold on
plot(time_s, vy_ref_log(:), 'b', 'LineWidth', 4)
plot(time_s, vy_cap_log(:), 'r', 'LineWidth', 2)
hold off
title('Commanded vs Tracked Y-Velocity')
xlabel('Time (sec)')
