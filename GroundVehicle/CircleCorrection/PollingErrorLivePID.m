...
for idx = 2:numFrames
    ...
    x(idx) = x_cap;  y(idx) = y_cap;

    % Console print (keep your style)
    fprintf('Name:"%s"  ', model.RigidBody(1).Name);
    fprintf('Frame #:"%d"  ', data.iFrame - frame_og);
    fprintf('X:%0.1fmm  ', data.RigidBodies(1).x * 1000); %Motive sends data in Meters, this converts to mm
    ...
    %% --- Map to wheel speeds --- %%
    w_cmd_rads = InverseKinematics([vx; vy; 0]);
