...
%% --- Orbit Parameters --- %%     
a     = 1.0;                % semi-major axis along x [m]
b     = 0.5;                % semi-minor axis along y [m]
T     = 15;                 % period [s]
% reference orbit parameters %
e     = sqrt(1-(b^2/a^2));  % eccentricity
mu    = (4*pi^2*a^3)/T^2;   % Kepler constant for given orbital period
...
t0 = tic;
    ...
    x_cap = -(data.RigidBodies(1).x - x_origin - a);  %%place robot on bottom right vertex (a , 0)
...
for k = 1:5
    ...
    x_ref = a * (cos(E) - e);
    y_ref = a * sqrt(1 - e^2) * sin(E);

    xr_log(idx) = x_ref;  yr_log(idx) = y_ref;

    % Reference body velocities for ROME path
    vx_ref = -a * sin(E) * (n / (1 - e*cos(E)));
    vy_ref =  a * sqrt(1 - e^2) * cos(E) * (n / (1 - e*cos(E)));
    ...
    ex = x_ref - x_cap;
    ey = y_ref - y_cap;
    abs_err = abs_err + sqrt(ex^2 + ey^2);
