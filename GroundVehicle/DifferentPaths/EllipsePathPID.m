...
%% --- ELLIPSE Parameters --- %%     
a     = 0.6;               % semi-major axis [m]
b     = 0.4;               % semi-minor axis [m]
T     = 15;                % period [s]
...
t0 = tic;
    ...
    x_cap = (data.RigidBodies(1).x - x_origin + a);
    ...
    %% --- Reference (ELLIPSE) position & velocity --- %%
    % Parametric ellipse: x(t) = a*sin(wt), y(t) = b*cos(wt)
    % Center at origin after zeroing (offset by 'a' in x like circle did)
    x_ref = -(a*sin(omega*t)); 
    y_ref = (b*cos(omega*t));
    xr_log(idx) = x_ref;  yr_log(idx) = y_ref;

    % Reference body velocities for ELLIPSE path
    vx_ref = -a*omega*cos(omega*t);
    vy_ref = -b*omega*sin(omega*t);
    ...
    Kp1 = 0.0000;   Ki1 = 0.0000;
    Vcorr = (Kp1 * Err) + (Ki1 * Err_int);
...
subtitle(sub_text);
title('Commanded vs Tracked Position (ELLIPSE)')
xlabel('x pos (m)')
ylabel('y pos (m)')
